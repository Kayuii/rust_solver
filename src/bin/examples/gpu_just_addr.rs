...
fn mnemonic_gpu(
    ...
    loop {
        ...
        let items: u64 = 16777216;

        let mnemonic_hi: cl_ulong = 0;
        let mnemonic_lo: cl_ulong = 1;
...
fn main() {
    ...
    let just_address_kernel: String = "just_address".to_string();
    let just_address_files = [
        "common", "ripemd", "sha2", "secp256k1_common", "secp256k1_scalar", "secp256k1_field", "secp256k1_group", "secp256k1_prec", "secp256k1", "address",
        "just_address",
    ];

    let kernel_name = just_address_kernel;

    let files = just_address_files.map(|f| format!("cl/{}.cl", f));
