...
fn main() {
    ...
    let just_seed_kernel: String = "just_seed".to_string();
    let just_seed_files = [
        "common", "sha2", "mnemonic_constants",
        "just_seed",
    ];

    let kernel_name = just_seed_kernel;

    let files = just_seed_files.map(|f| format!("cl/{}.cl", f));
